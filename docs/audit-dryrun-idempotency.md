...
| R2 | REST `POST /analyses/{id}/action` (`stop`) — `stop_analysis` | `handlers/analysis_sla.py:440` (gate line) | `dry_run = config.dry_run` / `if not dry_run:` (analysis_sla.py:438-440); message suffix `"— stop suppressed (spec.dryRun)"` + `AnalysisStopped` Warning Event (analysis_sla.py:444-445); anchor written via `StatusStore.set_stop_record` with `outcome="dry-run"` (analysis_sla.py:448-452). Post-#164: when `dry_run=True` the `EventEmitter.emit` call short-circuits — the `AnalysisStopped` Event is **not** posted to the kube-apiserver; `EventEmitter.suppressed_count` increments instead (#164). The composed message is still visible in the operator INFO log line. `openstudio_client.py:355` (definition line; stale `openstudio_client.py:209` corrected in same PR). | GATED |
...
```bash
ruff check . && pytest                       # both green (1270 tests across 57 files, current count)
...
local; the scrape is in-cluster). The exhaustive inventory — **22 counters + 18 gauges + 5 histograms** — is asserted by the canonical
