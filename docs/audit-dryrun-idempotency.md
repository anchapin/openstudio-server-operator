...
| Kubernetes Events (`kopf.event` / `create_namespaced_event`) | the `emit` seams in the five handler wrappers + `singleton.py:343` + the prune entrypoint's CoreV1 emitter (#78) | Observability objects — they ARE the dry-run substitution mechanism. (K8s may rate-limit them; they mutate no managed resource.) |
