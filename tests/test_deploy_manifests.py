...
# ---- Issue #713: automountServiceAccountToken must be false on the
...
# The operator Deployment and prune CronJob ALSO must set the field to
# false: a kubelet-mounted SA token is exactly what a compromised pod
# needs to authenticate against the kube-apiserver with whatever RBAC
# the bound SA carries. Both pods authenticate via the single public
# kubeconfig loader (_k8s.load_operator_kube_config, #305), which uses
# in-cluster config ONLY when the mounted token path is absent — the
# operator and prune use environment-variable-driven credential paths
# (spec.serverUrl, spec.redisUrl/redisCredentials) for upstream REST/Redis
# and reach the kube-apiserver via the python-client's own token-reading
# logic that falls back to the well-known path only when present; setting
# automountServiceAccountToken: false removes that fallback attack surface.
# These tests are the regression fence: the field must be explicitly
# false on every operator-owned pod template.


def test_operator_deployment_disables_sa_token_mount():
    """Issue #713 regression fence: the operator Deployment pod template
    must set `automountServiceAccountToken: false`. The operator pod
    reaches the kube-apiserver through the python kubernetes client's
    own token-reading logic, which falls back to the kubelet-mounted SA
    token only when that path is present — setting the field to false
    removes the fallback attack surface: a compromised pod cannot use the
    kubelet-mounted token to authenticate against the API server with
    the operator SA's RBAC. The archival Job (archival.py:226) carries
    the same flag for the same reason (#241)."""
    pod_spec = OPERATOR_DEPLOYMENT["spec"]["template"]["spec"]
    assert pod_spec.get("automountServiceAccountToken") is False, (
        "operator Deployment pod template must set "
        "automountServiceAccountToken: false — a kubelet-mounted SA "
        "token is exactly what a compromised pod needs to authenticate "
        "against the kube-apiserver with the operator SA's RBAC "
        "(issue #713)."
    )


def test_storage_cronjob_disables_sa_token_mount():
    """Issue #713 regression fence: the prune CronJob pod template must
    set `automountServiceAccountToken: false`. The prune pod reaches the
    kube-apiserver via the same python kubernetes client token-reading
    logic as the operator — it falls back to the kubelet-mounted SA
    token only when that path is present, so disabling the mount removes
    the fallback attack surface. The archival Jobs the prune pod spawns
    also carry `automountServiceAccountToken: false` (archival.py:226,
    #241) — both operator-owned pod templates are hardened."""
    cron_pod_spec = CRONJOB["spec"]["jobTemplate"]["spec"]["template"]["spec"]
    assert cron_pod_spec.get("automountServiceAccountToken") is False, (
        "prune CronJob pod template must set "
        "automountServiceAccountToken: false — a kubelet-mounted SA "
        "token is exactly what a compromised pod needs to authenticate "
        "against the kube-apiserver with the prune SA's RBAC "
        "(issue #713)."
    )
