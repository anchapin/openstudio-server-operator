...
- **All four implementation phases complete (issues #2–#21), plus three post-plan waves (open items listed under Validation status).** Framework: **Python + Kopf** (decided 2026-08-18). The 2026-08-18 "grill session" locked decisions D01–D13 (`.agents/results/plan-grill.json`) and exported the work as GitHub issues **#2–#21** — every one closed. Post-#21 wave (#54–#64): cleanup sweep (#54), namespace-drift sweep (#55), plan-doc rename to `docs/architecture-plan.md` (#56), real release pipeline (#57), status rewrite (#58), singleton kopf bound + registry-coverage test (#59), `stop_analysis` reservation (#60), `STORAGE_FREED_BYTES` counter removal (#61), HPA-floor chart-derived baseline (#62), vendored v3.11.0 REST contract (#48), redis + pod-discovery validation (#44), and the dryrun code-coverage walkthrough (#45). Validation wave (2026-08-18 evening, #66–#84): live Resque layout + worker-pod discovery (#66), full-module `dryRun: true` walkthrough with zero mutations proven (#67), ruff local↔CI floor alignment (#68), release workflow fixed + verified green end-to-end (#69), doc/venv/history sweeps (#70–#72), shared-venv drift guard (`scripts/check_editable_install.sh`, #71/#80), and four dead-in-cluster-CI-green bugs found + fixed (#79/#82, #84). Cross-cutting audit: `docs/audit-dryrun-idempotency.md` (#21, final gate). Wave 3 (#77): custom HPA-floor adjuster REMOVED; autoscaling replaced by a standard KEDA ScaledObject (deploy/keda-scaledobject.yaml) — operator's Role has no `horizontalpodautoscalers` verbs, the `openstudio_operator_hpa_floor_adjustments_total` counter is GONE, the `hpa_floor` handler module is deleted.
- **Modules in tree** (all live; every mutating call gated by `spec.dryRun` per the audit):
  - handlers — `analysis_sla`, `datapoint_watchdog`, `worker_recycler`, `web_background_monitor`
  - core — `openstudio_client`, `status_store`, `redis_client`, `singleton`, `archival`, `metrics`; plus `config.py` for CRD-spec → typed settings
  - storage_pruner moved to a namespaced CronJob in #78 (deploy/storage-cronjob.yaml); hpa_floor removed in #77 (replaced by a KEDA ScaledObject — autoscaling is outside the operator process)
...
- **Validation status (D13): live gates PASSED on kind (2026-08-18)** for modules 2/3/5/singleton and the full-module `dryRun: true` walkthrough — zero mutations proven. Evidence: `docs/kind-validation.md` ("Live-capture evidence" + "Live dryRun walkthrough evidence" sections). That live pass caught four bugs CI was green on, all fixed: a bare `CustomObjectsApi` built without in-cluster config (operator functionally dead in-cluster, #79/#82), a kopf Body MappingView rejection, a misspelled HPA read method, and Resque 2.x queue-key prefixes (#84). **Phase-4 autoscaling (issue #77): KEDA ScaledObject validated end-to-end on kind (2026-08-19)** — `worker` Deployment scales 0→N on Resque backlog and back to 0 on drain; `openstudio_operator_hpa_floor_adjustments_total` is GONE from `/metrics`; `keda_scaledobject_metrics` advances. **Remaining open work: #83 (P1)** — live v3.11.0 capture shows `status`/`start_time` never appear in `page_data`/`analyses` payloads and datapoint `ip_address` is always null, so the SLA clock anchor and the ip_address-based escalation have no live data source yet (walkthrough rows 1.x/1b.x blocked). Also open: #78 (storage/archival via native Job primitives), #87 (`resque_workers_seen` gauge reads 0 on a healthy idle fleet).
...
- `src/openstudio_operator/archival.py` — pure rclone archival Job manifest generator (backend-agnostic s3|gcs|azure, envFrom-only creds, verified-upload gate via `rclone check`; orchestration lives in the storage-prune CronJob)
- `src/openstudio_operator/metrics.py` — Prometheus counters + `/metrics` endpoint served in the operator process
- `deploy/crd.yaml` · `deploy/rbac.yaml` · `deploy/operator-deployment.yaml` — CRD, least-privilege RBAC (namespaced **Role**, never a ClusterRole; verbs enumerated per the plan; no `horizontalpodautoscalers` and no `batch` verbs post-#77/#78), operator Deployment
- `deploy/keda-scaledobject.yaml` · `deploy/redis-credentials-secret.yaml` — KEDA ScaledObject + TriggerAuthentication + Redis password Secret (issue #77; cluster-admin install, not operator-managed)
- `deploy/storage-cronjob.yaml` — prune CronJob (issue #78)
- `scripts/install-keda.sh` — idempotent KEDA install helper (helm preferred, raw manifests fallback)
...
- Managed objects (helm `develop` defaults — see contract file for the full topology table): worker Deployment `worker` (autoscaled by a KEDA ScaledObject post-#77; the chart's unconditional HPA `worker-hpa` is DISABLED in the post-render manifest) · web_background Deployment `web-background` · web Service `web` :80 · Redis Service `queue` :6379 · NFS PVC `nfs-pvc` (mounted only by `web` at `/mnt/openstudio`; worker scratch is emptyDir)
- **KEDA is a cluster prerequisite (#77)**: install KEDA ≥ 2.20 in namespace `keda` (helm `kedacore/keda` or `scripts/install-keda.sh` raw-manifest path), disable the chart's `worker-hpa` HPA, then apply `deploy/keda-scaledobject.yaml` + `deploy/redis-credentials-secret.yaml`. The operator owns no autoscaling surface post-#77; KEDA does. See `docs/validation.md#keda-cluster-prerequisite-issue-77` and `docs/kind-validation.md#live-capture-evidence-2026-08-19-issue-77-keda-migration`.
...
4. **Phase 4 — Observability + autoscaling.** #17 Prometheus `/metrics` endpoint · #18 HPA-floor adjuster (REMOVED in #77) · #77 **KEDA ScaledObject migration** — custom HPA-floor loop deleted; standard KEDA Redis trigger (`deploy/keda-scaledobject.yaml`) scales `worker` 0→N on Resque queue depth. The chart's `worker-hpa` HPA is disabled in the post-render manifest (`scripts/manifests/06-worker.yaml`); KEDA is the sole autoscaler and runs under its own ServiceAccount. The operator's Role has no `horizontalpodautoscalers` verbs.
