...
- **API ground truth is `.agents/skills/_shared/api-contracts/openstudio-server-v3.11.0-rest.md`**, verified against NREL/OpenStudio-server tag `v3.11.0` and the NatLabRockies helm chart `develop` branch. The plan doc (`docs/architecture-plan.md`) predates verification; where it conflicts with the contract file, **the contract file wins** (see its "Non-existent / legacy" section for the traps: no `PUT action`, no `kill`/`hard_stop`, no `/cluster.json`).
