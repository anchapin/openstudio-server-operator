...
- **All four implementation phases complete (issues #2–#21), plus the wave-1a follow-ups (#54–#57).** Framework: **Python + Kopf** (decided 2026-08-18). The 2026-08-18 "grill session" locked decisions D01–D13 (`.agents/results/plan-grill.json`) and exported the work as GitHub issues **#2–#21** — every one closed. Post-#21: post-implementation cleanup sweep (#54), namespace-drift sweep in README/Dockerfile (#55), plan-doc rename to `docs/architecture-plan.md` (#56), and the real release pipeline (#57). Cross-cutting audit: `docs/audit-dryrun-idempotency.md` (#21, final gate).
- **Modules in tree** (all live; every mutating call gated by `spec.dryRun` per the audit):
  - handlers — `analysis_sla` (#8/#9), `datapoint_watchdog` (#10), `worker_recycler` (#11), `web_background_monitor` (#13), `storage_pruner` (#15/#16), `hpa_floor` (#18)
  - core — `openstudio_client` (#6), `status_store` (#7), `redis_client` (#12), `singleton` (#14), `archival` (#15), `metrics` (#17); plus `config.py` (#5) for CRD-spec → typed settings
- **API ground truth is `.agents/skills/_shared/api-contracts/openstudio-server-v3.11.0-rest.md`**, verified against NREL/OpenStudio-server tag `v3.11.0` and the NatLabRockies helm chart `develop` branch. The plan doc (`docs/architecture-plan.md`) predates verification; where they conflict, **the contract file wins** (see its "Non-existent / legacy" section for the traps: no `PUT action`, no `kill`/`hard_stop`, no `/cluster.json`). In-repo vendoring of the contract + the kind live-drift findings tracked in **#48**.
- **Operator image + release:** `ghcr.io/anchapin/openstudio-server-operator:dev` is now published automatically on every push to `develop`; `v*` tags publish a versioned image and create a GitHub Release (`.github/workflows/release.yml`, #57, `linux/amd64`). Tests are still mocked — CI green does not prove behavior against a real cluster. Validation strategy (D13): kind + real `3.11.0` images on a dev machine (#19), then a work-cluster dry-run runbook (#20).
- **Validation status (D13):** kind cluster recipe + fixture capture (#19) and the work-cluster runbook (`docs/validation.md`, #20) are merged. The kind `dryRun: true` smoke walkthrough steps 1–2 are **live-verified** (CRD/RBAC apply, dry-run CR accepted with defaults, client reads succeed); steps 3–4 (full-module smoke on kind) are tracked in **#45**. **The actual next steps are:** **#44** (validate the Resque Redis key layout + worker-pod discovery against live 3.11.0) and **#45** (complete the kind full-module `dryRun: true` smoke walkthrough).
