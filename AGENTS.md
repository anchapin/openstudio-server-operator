...
- **Merge-subject hygiene (issue #88):** `develop` is the default branch — GitHub closes an issue when **any commit** with closing keywords (`Closes #N` / `resolve #N` / `fixes #N`) lands on `develop`, *including* auto-generated squash-merge subjects. For keep-open PRs use `Refs #N` / `for #N` / `touches #N` in the PR title AND the squash subject, then `gh pr merge N --squash --subject "…" --body "Refs #N"` — never trust the auto-generated subject. When the issue SHOULD close on merge, the closing keyword IS desired in both subject (`fix: resolve #N — …`) and PR body (`Closes #N`).
