...
.venv/bin/pytest # 520 tests across 27 files — use the venv pytest (system pytest won't resolve `openstudio_operator`)
...
- `src/openstudio_operator/handlers/` — Kopf handlers, one file per plan module: `analysis_sla`, `datapoint_watchdog`, `worker_recycler`, `web_background_monitor`. `handlers/__init__.py` is the operator entrypoint: it starts the Prometheus `/metrics` server (`#17`), installs the singleton guard (`#14`), and registers the warning-event sinks for the Redis-URL guard (`#116`), Resque key-layout guard (`#163`), and status-store cap (`#171`) — all three use the same queue/drain pattern (emit from non-kopf contexts, drain on the next OSCM `@kopf.on.event` tick). Add new handler modules to its import block and the gate picks them up automatically.
