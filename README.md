...
action counters, the tick-duration histograms, the Resque queue gauges
+ freshness, and the per-module scheduler heartbeat lag (datasource
...
adding a counter, gauge, or histogram here without adding it there (or vice
  versa) fails CI loudly. **Current shape: 24 counters + 18 gauges + 5 histograms
...
post-#570 `singleton_expected_handlers` boot-time expected-count gauge
(the wrap-gap denominator — makes a partial singleton-guard unwrap
alertable, not just a total one);
post-#488 Redis + kube-api request-duration histograms;
post-#490 `redis_key_layout_status_fresh` freshness pair + periodic
revalidation).**
...
- `openstudio_operator_redis_request_duration_seconds` (histogram, #488) —
  per-`operation` (`llen` | `smembers` | `scan` | `exists`) wall-clock
  duration of ReadOnlyRedisClient read methods (`exists` added by #688 for
  the layout validator's O(1) verdict probes).
...
- `openstudio_operator_metrics_server_bound` (gauge, #393) — outcome of
  the /metrics server's first bind attempt (per `(addr, port)`); `== 0` is the
  canonical "Prometheus scrape is down because of US" signal.
- `openstudio_operator_handler_last_tick_timestamp` (gauge, #469) — the
  scheduler heartbeat: per-`module` Unix-epoch timestamp of the most
...
- `openstudio_operator_singleton_wrapped_handlers` (gauge, #491) — the
  boot-time record of how many OSCM timers the singleton guard actually
  wrapped; `== 0` on a booted operator that expects timers is the
  silent-unwrap failure mode (see the metrics table row and the
  `OpenStudioOperatorSingletonGuardUnwrapped` alert).
- `openstudio_operator_singleton_expected_handlers` (gauge, #570) — the
  boot-time record of how many OSCM timers the singleton guard EXPECTED
  to wrap (the population kopf reports; the Python-registry count on the
  internals-mismatch branch). Paired with the wrap count it makes a
  PARTIAL unwrap — `wrapped < expected`, e.g. 3 of 4 — alertable
  (`OpenStudioOperatorSingletonGuardUnwrapped` rekeyed onto strict `<`,
  which subsumes the old `== 0` clause).
...
| `openstudio_operator_stops_total{outcome}` | counter (labelled) | SLA handler · #707 | Waiting-variant stop issued (or dry-run / completed / timeout outcome). Labelled by `outcome` ∈ {`"issued"`, `"dry-run"`, `"completed"`, `"timeout"`}. |
...
| `openstudio_operator_redis_key_layout_status_fresh` | gauge | `handlers` (`_set_redis_key_layout_status` in lockstep with the status gauge, at every `_check_redis_key_layout_for_cr` run — the `@kopf.on.event` watch plus the #490 revalidation riding the web_background stall tick) · #490 | Last-run Unix timestamp for the `redis_key_layout_status` data gauge. Set to `time.time()` on EVERY terminal path (`ok` \| `degraded` \| `unreachable` \| `error` \| `skipped` — fresh means recently validated; the status gauge carries the result). Pre-#490 the status gauge was set only by the `@kopf.on.event` watch (boot listing + CR edits), so a steady-state cluster generated no watch events and the gauge held its boot value indefinitely — a mid-flight Resque layout drift (helm chart upgrade to a different prefix, queue backend swap) read `ok` while `web_background_monitor`'s leg-A/leg-2 evaluation ran against a stale layout. Since #490 the `web_background_monitor` timer re-runs the check at most once per `REDIS_KEY_LAYOUT_REVALIDATION_INTERVAL` (5 min, `_constants.py`), bounding the drift-detection window. Dashboard query: `time() - openstudio_operator_redis_key_layout_status_fresh` — **alert on `> 600` (2 × the 5-minute revalidation interval)**. |
...
| `openstudio_operator_singleton_wrapped_handlers` | gauge | `singleton` (`install_singleton_guard` end-of-install stamp) · #491 | **The runtime kopf-pin fence** — the number of OSCM spawning (timer/daemon) handlers the singleton guard actually wrapped at boot, counted as registry entries whose fn carries the gate marker (an idempotent re-install wraps nothing new but keeps reporting the gated population; handlers skipped for missing #250 registration are correctly absent). The gate reaches into kopf's private `registry._spawning._handlers` — the reason kopf is pinned `>=1.37,<1.45`; a kopf upgrade that moves that internal makes the gate wrap NOTHING and return 0, silently disabling D05 enforcement while the operator appears healthy (every timer still fires, ungated). The registry-coverage CI test fences the layout at build time; this gauge is what Prometheus can alert on. **Alert on `== 0` sustained on a booted operator that expects timers** (shipped as `OpenStudioOperatorSingletonGuardUnwrapped`, `for: 5m`) — the complement of the #469 scheduler heartbeat: the heartbeat proves the scheduler is invoking the timers, this gauge proves those invocations are guarded. Unlabelled (one series, process-wide); set at boot wiring time, before any dry-run-gated action could exist — D11-exempt. |
| `openstudio_operator_singleton_expected_handlers` | gauge | `singleton` (`install_singleton_guard` pre-wrap scan stamp) · #570 | **The wrap-gap denominator** — the number of OSCM spawning handlers the singleton guard EXPECTED to wrap at boot, sized from the population kopf actually reports (every `registry._spawning._handlers` entry whose selector matches the OSCM resource and whose fn is not None, counted BEFORE wrapping; on the registry-internals-mismatch branch the Python-level `_oscm_handlers.REGISTRY` count stands in, so the kopf-upgrade total-unwrap shape keeps reading `0 < N`). Alone, the #491 wrap count cannot express "3 is wrong" — a PARTIAL unwrap (one handler skipped for missing #250 registration, or a `dataclasses.replace` TypeError) left a kopf-registered OSCM timer running UN-GATED while the wrap gauge read a plausible "3 of 4" and the `== 0` alert stayed silent; in a multi-CR namespace that ungated timer double-soft-stops, double-requeues, and double-restarts on the loser CR — precisely the D05 violation. **Alert on `openstudio_operator_singleton_wrapped_handlers < openstudio_operator_singleton_expected_handlers` sustained** (the rekeyed `OpenStudioOperatorSingletonGuardUnwrapped`, `for: 5m`) — strict `<` subsumes the historical `== 0` clause. Unlabelled (one series, process-wide); set at boot wiring time like the #491 gauge — D11-exempt. |
...
│   ├── service-metrics.yaml    # ClusterIP Service fronting the operator's named port `metrics` (:9090) — alert discovery half (#682)
│   ├── servicemonitor-metrics.yaml  # ServiceMonitor wiring Prometheus Operator discovery to the metrics Service (#682; rename `release:` per install)
...
│   ├── metrics.py              # Prometheus counters + gauges + histograms + /metrics endpoint (24+18+5)
